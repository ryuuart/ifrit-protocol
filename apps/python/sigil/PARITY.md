...
| `compose::connect` wire operators and routers | Compose | 8 |
...
| Composer | Checked `ctx.composer`: `render`, `renderSlot`, `bounds`, `hitTest`, `settling`, `active`, `dirty`, `purgeCaches` and owned `stats` | `paragraphLayout`, `beatsOf`, `units` and `cascadeSpanMs`; `setInherited`, `setView` and `declareInputSpace`; `setProfiling` with `profile`, `setCompositeCounting`, `setAutoTexturePromotion` and `setBakeDensity`; a standalone `compose::Composer` driven through `setSize`, `setClock`, `draw`, `setPointer` and `setKey` |
...
| Surface | Bound | Not yet bound |
| --- | --- | --- |
| Elements | Factories `box`, `stack`, `positioned`, `text` over a string, a styled string or rich text, `frame`, `image` over a Skia image, `picture`, `pathFigure`, `slot`, `memo`, keyed `pen` and `graphics` with Python programs, and `layout` over the six stock schemes, an operator or a Python operator; `point`, the node with no extent that carries a key and facts; `drawWith`, keyed or not, over a Python program handed one pen and the settled scope; every native Element verb except the five named opposite, with named arguments throughout, `padding` and `margin` as three overloads of one, two or four named lengths: layout and placement, 2D and 3D transforms, fills, corners, shapes and clips, effects, blending and hit testing, opacity, entrances and transitions, caching, the cascade verbs `font`, `block`, `ink`, `styleSheet`, `styleClass`, `role`, `var` and `applyStyleSheet`, the fact verbs `attribute` and `attributes` over an `Attributes` table, `operators` over the stock arranging values, the connecting records and Python objects that `arrange(Arrangement)` or `add(Scope)`, and the text-leaf verbs; dimensions in px, pct, em, rem, lh, var, pw and ph; children as elements or one iterable; `band` over a shape at an `across` width, with `bandAlignment` | `Element::fx` tracks and the `fx::` text effects (`scramble`, `keys`, `hold`, `sequence`, `mix` and `variableAxis`), `mask`, `variationDrive` and `mark`; keyed `shape(key, function)`; keyless `pen(program)` and `graphics(program)`; `custom` paint programs and pen programs that read `PaintContext`; `image(ImageAsset)`; `text(Paragraph)`; the `connect::Along`, `pin::`, `outline::` and `stamp::` operators, with `Anchor` and `RailRouter`; `feed`; `instancing` pools and atlases; `Pattern`; `Table`, and a Python placement scheme of the older `place(LayoutInput)` shape, which an arranging operator stands in for; shader fills and gradient helpers; standalone `intrinsicSize`, `snapshot`, `metrics`, `measureRun`, `runPens`, `atCapHeight` and `fitRun`; `tiles` and `shelve`; `video` and `web` leaves; `TextureScene` and `texture`, which paint a tree into a material texture |
| Selector sheets | `compose.selector(cssText)` and the same value built name by name under `compose.select` (`styleClass`, `role`, `any`, `is_`, `where`, `notAnyOf`); the four combinators as `child`, `descendant`, `next` and `sibling`; every structural pseudo-class, with `nthChild` and `nthLastChild` taking `of`; `\|`, `&` and `~` for the list, the compound and the negation CSS's `!` spells; `specificity()` over a comparable, ordered `Specificity` and `matchesNothing()`; `compose.Rule` from either door with `font`, `block`, `ink` and `var`, reading its selector, type and block back; `compose.StyleSheet` from an iterable of rules and sheets, with `rules`, `len`, iteration, `+` and value equality; `Element.applyStyleSheet`, whose sheet sees only its own subtree | `Rule::inkVar` and `Rule::vars`, which name a `VarTable` registered later in the assembly; a hash on a selector, a rule or a sheet, since nothing reads a selector's text back for one to agree with; `:has()`, `calc()` in a length, a transition stated in a rule and the box half of what a rule can state, none of which the native grammar carries yet |
...
| `hit_slots` | Slots; hit test and bounds queries; wires keyed by the pair they join; star and blob silhouettes; readout rows | `sketch::kit::readout`; `compose::connect` wire operators and routers; `compose::routers` stock routers; `geometry::shapes` silhouette generators |
...
| `routers_straight` | Wires with stock routers (straight, orthogonal bends, arc); a run of stops with an octilinear router; path formats | `sketch::kit::sectionHeader`; `compose::connect` wire operators and routers; `compose::routers` stock routers |
| `routes_probe` | Wires with routers; standalone profiling composer; a wire asked for by its pair's key; cache verdict profile rows; pen leaf; readout classes | `sketch::kit::sectionHeader`; `compose::connect` wire operators and routers; Standalone `compose::Composer` over a caller canvas; `compose::Composer::profile` node costs; `compose::routers` stock routers; `SkSurfaces::Raster` standalone raster surfaces |
...
| `passive tree` | SDF sockets with bound glow uniform; wires with the orbit router; layered brush presets; stock silhouettes; span trims; legend; title card | `sketch::kit::titleCard`; `sketch::kit::legend` and `swatchStrip`; `compose::connect` wire operators and routers; `compose::lines::presets` and `compose::brush::presets`; `compose::routers` stock routers; `geometry::shapes` silhouette generators; `material::sdf` shapes and styles; `material::skia::Paint` motion-driven uniforms and offsets |
...
| Sketch | Needs | Not yet bound |
| --- | --- | --- |
| `flourish` | Ornament and flourish kits; contour walks; nine-slice; paint programs; routed wires; layer blur; runtime shaders; silhouettes; pen overlays | `compose::onEdges` and `compose::inset` adaptors; `compose::connect` wire operators and routers; `compose::PaintProgram` canvas decorations; `compose::Slice` nine-slice decoration; `compose::ContourWalk` path-walk stamps; `compose::routers` stock routers; `compose::kit::ornament` palettes, frames and marks; `compose::kit::flourish` vines and cards; `SkSurfaces::Raster` standalone raster surfaces; `geometry::shapes` silhouette generators; `geometry::shapes` corner operators |
| `night network` | Compose brushes and lines; stroke presets; path shapers; routed wires; span masks; SDF recipes; bound paint uniforms; art warp | `compose::keyedShape` comparable shapes; `compose::Element::mask` and `compose::by` gates; `compose::lines::Line` and `lines::Rails`; `compose::connect` wire operators and routers; `compose::Brush` shaped layer stacks; `compose::brush` scatter, pattern and art brushes; `compose::lines::presets` and `compose::brush::presets`; `compose::routers` stock routers; `geometry::shapes` silhouette generators; `geometry::shapers` and `path::Shaper`; `material::sdf` shapes and styles; `material::skia::Paint` motion-driven uniforms and offsets |
...
| `ds2_bench` | Routed wires; spans mask; layered brushes; kinetic text; instanced atlas; SDF materials; live shader uniforms; stock shapes | `compose::Element::fx` text tracks; `compose::instancing` atlases, pools and instances; `compose::Element::mask` and `compose::by` gates; `compose::connect` wire operators and routers; `compose::LayeredBrush` stroke layers; `compose::fx` kinetic presets (`rise`, `pop`, `typeOn`); `compose::lines::presets` and `compose::brush::presets`; `compose::instancing::place` placers; `motion::Spread` and `motion::Cascade` schedules; `geometry::shapes` silhouette generators; `geometry::path::PolarFrame` and `path::Grid`; `geometry::shapes::ticks` and `chords` divisions; `material::sdf` shapes and styles; `material::skia::Paint` motion-driven uniforms and offsets |
