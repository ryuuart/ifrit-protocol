...
## A study's verification pass costs nine seconds of setup, per open

`minard_1869`'s `setup()` runs for 9.1 s, and `runAudits()` is 8.5 s of
it — the whole of it, measured phase by phase:

| what | ms |
| --- | ---: |
| `test::widthAlong(advBand, advSpine, …)` | 3106 |
| `test::coverage(advance, bounds, 512)` | 2061 |
| `test::coverage(advance ∪ retreat, region, 384)` | 1583 |
| `test::coverage(retreat, bounds, 512)` | 1332 |
| the retreat band's own build + its width audit | 430 |
| `Simplify` of the 719-step advance band, and the contour walks | 18 |
| everything else in `runAudits` (fits, haversines, connectivity) | ~0 |
| faces, patterns, tickers, describe, render | 12 |

Nothing here is recomputation and nothing can be cached: each figure is
measured once. The cost is the two verification routines themselves, and
both are quadratic in the wrong term for a band:

* `coverage` calls `SkPath::contains(x, y)` per lattice point, and
  `contains` walks every verb of the path. A band is one quadrilateral
  per sampled step — 719 of them here — so a 512×512 lattice is 262,144
  walks of ~2,900 verbs. The fix at the origin is to resolve each piece's
  coverage ONCE (a raster mask, or a y-bucketed edge list) and read the
  lattice out of it, rather than asking the path 262,144 separate
  questions.
* `widthAlong` flattens the band to ~50,000 unit-length edges and then
  tests every edge for every ray — 325 stations × 90 directions. Its own
  doc states the O() and calls it a verification pass, which it is; but
  the ray reaches `profile.max() * 4 + 40` px and every edge outside that
  radius of the station cannot contribute a crossing inside the window
  the chord is read from. A spatial cull on the station point is exact
  and would leave a few hundred edges per ray instead of fifty thousand.

Both belong in `compose/testing/Checks.h`, and the test to assert once
they are fixed is the one that already exists in
`ComposeTestStrokeGrammar.cpp`: the same band, the same numbers, faster.
The consumer cost is paid on every open — every plate render, every
thumbnail fill, every hot reload of the sketch.
