...
// Forward-declared Qt RHI types + pimpl keep ObjC out of this header so it's includable from plain C++.
class QRhi;
class QRhiTexture;
...
public:
  ...
  /**
   * Appends a Syphon blit to the still-open @p cb for the given texture region.
   * No-op when no Syphon clients are connected.
   */
  void publishFrame(QRhiTexture *texture, QRhiCommandBuffer *cb, int width,
                    int height);
...
private:
  std::string m_name;
  std::unique_ptr<SyphonBridgePrivate> m_syphon_bridge;
};
