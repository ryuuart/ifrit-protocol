...
| Named paragraph styles | exists — the same registry shape for blocks, whole rather than partial because a block style's overridable settings are already optional | `ParagraphStyleSheet` | `Element::paragraphs(names)` against `env::Provide<weave::ParagraphStyleSheet>`; a name no sheet carries warns |
