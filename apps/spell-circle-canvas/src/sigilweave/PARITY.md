...
| Named character styles | exists — a base style and the named PARTIALS over it, whose lookup always answers: an entry states what it CHANGES, so one sheet serves a document whose base size was decided elsewhere | `StyleSheet`, `RichText::add(text, name)`, `RichText::styles` | `weave::rich().add(text, name)`, with `env::Provide<weave::StyleSheet>` supplying the sheet a value did not name |
| Named paragraph styles | exists — the same registry shape for blocks, whole rather than partial because a block style's overridable settings are already optional | the block half of a `StyleSheet` rule | `Element::paragraphs(names)` against `env::Provide<weave::StyleSheet (block half)>`; a name no sheet carries warns |
| A style stated in PART, inherited for the rest | done — every field of `Type` is optional, so a call site states the two it changes; it carries the whole inherited set, the shaping controls and the paint around the colour included; `overlay` resolves one partial against what it inherits from, `merge` folds two partials written about one passage into one, and `initialType` is what an unset field means when nothing is left above it | `Type`, `overlay`, `merge`, `initialType`, `toTextStyle`, `reshapes` | a partial on an element, resolved against the style the tree hands down |
| A block stated in PART, inherited for the rest | done — every field of `Block` is optional: leading, alignment, justification, hyphenation, tab stops, first- and last-line indent, widows and orphans, balanced ragging, writing mode and line-break locale, the fields a block takes from the passage it stands in; `overlay` resolves one onto a whole `ParagraphStyle`, `merge` folds two into one, and a the block half of a `StyleSheet` rule holds them under names | `Block`, `overlay`, `merge`, `toParagraphStyle`, `apply`, the block half of a `StyleSheet` rule | a partial on any element, inherited down the tree; the block sheet's names resolved over it |
