...
| A style stated in PART, inherited for the rest | done — every field of `Type` is optional, so a call site states the two it changes; `overlay` resolves one partial against what it inherits from, `merge` folds two partials written about one passage into one, and `initialType` is what an unset field means when nothing is left above it | `Type`, `overlay`, `merge`, `initialType`, `toTextStyle` | a partial on an element, resolved against the style the tree hands down |
