/** @file
 * Range search over a paragraph — substring, word and ICU regular
 * expression, whole or scoped to a window — and the marker set that keeps
 * named ranges in step with the paragraph's edit log.
 */
...
std::optional<std::vector<CharRange>> findRegexMatches(
  ...
  if (U_FAILURE(status) || !regularExpression) {
    if (regularExpression) uregex_close(regularExpression);
    return std::nullopt;
  }
  std::vector<CharRange> matches;
  scope = clampScope(paragraph, scope);
...
const std::vector<CharRange>* MarkerSet::rangesFor(
    std::string_view name) const {
  for (const auto& marker : m_markers)
    if (marker.first == name) return &marker.second;
  return nullptr;
}
...
bool MarkerSet::synchronize(const Paragraph& paragraph) {
  if (paragraph.revision() == m_revision) return true;
  ...
  if (!historyAvailable) {
    for (auto& markerEntry : m_markers) markerEntry.second.clear();
    return false;
  }
