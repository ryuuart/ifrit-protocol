 ...
 * @ingroup document
 *
 * Minimal inline-storage vector used by SigilWeave's public value types
...
class InlineVector {
 ...
 public:
  ...
  void reserve(size_t requestedCapacity) {
    if (requestedCapacity > m_capacity) grow(requestedCapacity);
  }
 ...
 private:
  ...
  void destroyElements() noexcept {
    for (size_t index = 0; index < m_size; ++index) data()[index].~T();
  }

  /** Destroys elements and returns to the empty, inline-capacity state. */
  void reset() noexcept {
    destroyElements();
    if (m_heap) std::allocator<T>().deallocate(m_heap, m_capacity);
    m_heap = nullptr;
    m_size = 0;
