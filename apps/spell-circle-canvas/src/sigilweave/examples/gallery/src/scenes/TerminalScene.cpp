...
class TerminalScene final : public Scene {
 public:
  FrameStats render(SkCanvas* canvas, SkISize size, double elapsedSeconds,
    ...
    const SkRect box =
        SkRect::MakeXYWH(48, 36, canvasWidth - 96, canvasHeight - 84);
    ...
    for (const PositionedRun& run : m_layout.runs) {
      ...
      for (size_t glyph = 0; glyph < shaped.glyphs.size(); ++glyph) {
        ...
        if (revealAt > elapsedChars) continue;  // not typed yet
        ...
        if (scrambling) {
          ...
          brightness =
              ...
              0.18f *
                  hash01(index * 513u +
                         static_cast<uint32_t>(elapsedSeconds / 0.055) * 77u);
        } else {
          const float age =
          ...
          brightness +=
              0.05f * std::sin(static_cast<float>(elapsedSeconds) * 11.0f +
    ...
    if (glowAmount > 0.01f) {
      ...
      m_buckets.drawEach([&](const auto& bucket) {
        ...
        glowPaint.setColor(SkColorSetARGB(
            static_cast<U8CPU>(
                std::min(255.0f, 235.0f * glowAmount * t * t * fade * fade)),
            64, 255, 128));
 ...
 private:
  ...
  void drawCursor(SkCanvas* canvas, SkPoint pen, float fontSize,
    ...
    const SkRect block =
        SkRect::MakeXYWH(pen.fX + 2.0f, pen.fY - fontSize * 0.78f,
                         fontSize * 0.55f, fontSize * 0.95f);
    SkPaint paint;
    paint.setAntiAlias(true);
    if (glowAmount > 0.01f) {
      paint.setColor(
          SkColorSetARGB(static_cast<U8CPU>(200 * glowAmount), 64, 255, 128));
      paint.setMaskFilter(
  ...
  void drawCrtOverlays(SkCanvas* canvas, SkISize size, double elapsedSeconds) {
...
SceneDescriptor makeTerminalDescriptor() {
  ...
  descriptor.parameters = {
      {QStringLiteral("speed"),
       QStringLiteral("Typing speed"),
       SceneParameter::Type::kFloat,
       60.0,
       8,
       320,
       QStringLiteral(" cps"),
      ...
      {QStringLiteral("scanlines"),
       QStringLiteral("Scanlines & CRT"),
       SceneParameter::Type::kBool,
       true,
       0,
       1,
       {},
