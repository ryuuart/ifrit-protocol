...
auto DunhuangStarChart::mapFrame(int k, int seg) -> Element {
  ...
  for (int t = -24; t <= 24; t += 6) {
    ...
    g.children({box()
                    ...
                    .fill(Fill::color(hexColor(0x8a7458, 0.40f)))});
    if (big)
      g.children({text(kit::formatted("%d", (int)std::lround(wrap360(
                                                mapCentre(k) + (float)t))))
  ...
  g.children({text(kit::formatted("%d", k))
  ...
  for (int m = 0; m < 28; ++m) {
    ...
    g.children({box()
                    ...
                    .stroke(PathFormat{
                        ...
                        .trimStart = 0.06f,
                        .trimEnd = 0.94f})});
    g.children({text(cat.xiu(m).native)
                    ...
                    .width(18)
                    .block({.alignment = weave::TextAlignment::kCenter})});
    g.children({text(cat.xiu(m).pinyin)
  ...
  for (int c = 0; c < nCols; ++c) {
    ...
    for (int gI = 0; gI < glyphs; ++gI) {
      ...
      g.children({box()
                      ...
                      .shape(shapes::blob((uint32_t)(k * 97 + c * 13 + gI),
                                          0.42f, 7))});
    }
  }
  return g;
...
auto DunhuangStarChart::discNotes(int seg) -> Element {
  ...
  for (int i = 0; i < 4; ++i)
    g.children({text(rows[(size_t)i])
                    ...
                    .top((float)i * 10.4f)
                    .width(rOut * 2 + 16)});
  g.children({text("slightly erased, sits near it — \"could be "
                   "the Pole "
                   "star\". Drawn as found.")
...
auto DunhuangStarChart::raRuler(int seg) -> Element {
  ...
  if (seg == 1)
    g.children({text("EACH FRAME SPANS 48° OF RA ON A 30° "
                     "PITCH · "
                    ...
                    .left(600)
                    .top(y - 26)
                    .width(900)});  // just clear of the sheet
  for (int k = 1; k <= 12; ++k) {
    const float s0 = mapSlotS(k);
    ...
    g.children({box()
                    ...
                    .stroke(lines::Line{
                        .width = 1.0f,
                        .fill = Fill::color(hexColor(0xc9a35c, 0.7f))})});
    g.children({text(kit::formatted("%d°", (int)std::lround(
                                               wrap360(mapCentre(k) - 24.0f))))
