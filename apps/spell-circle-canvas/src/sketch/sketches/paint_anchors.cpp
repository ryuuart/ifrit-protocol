 ...
 * on a subtree with each anchor, and a canvas-anchored fill running
 * through a row of cards as though they were windows cut in it.
 */
 ...
 *  square so that every anchor is a statement about the BOX and never
 *  about the ramp. */
paint::Paint ramp() {
 ...
 *  between the panels is the anchor alone. */
Element subtree(PaintAnchor anchor) {
...
/** A row of cards, each filled with the SAME paint anchored to the
  ...
  void setup(sketch::SketchContext& ctx) {
    ...
    ctx.composer.render(sketch::kit::page(
        {.title = "The box a paint is stretched over",
         .subtitle =
             "One unit-square ramp · the ink on a subtree under each "
...
SIGIL_SKETCH(PaintAnchors, "Study · Paint",
             "one unit-square ramp read through every paint anchor — the ink "
             "on a subtree and a canvas-anchored fill across three cards")
