...
auto ChaucerAstrolabe::plate() -> Element {
  ...
  // the case where it is not.)
  {
    ...
    for (int i = 1; i <= 5; ++i) {
      ...
      for (int s = -1; s <= 1; s += 2) {
        ...
        sky.children(
            {ring(local(az.centre.x, az.centre.y), rad,
  ...
  // region is built with a raw SkPathOp below the Compose seam.
  {
    ...
    for (int k = 1; k <= 11; ++k) {
      ...
      night.children(
          {ring(PL(c->centre.x, c->centre.y), rad,
  ...
  g.children({cut({0, almCy(0.0f)}, almR(0.0f), 3.4f, 0.90f, 0.50f, "horizon")
  ...
  // --- the zenith ------------------------------------------------------
  g.children(
      {dot(PL(0, kYzen), 3.6f, Fill::color(hexColor(0x3a2a10, 0.85f)))
           .key("zenith")
  ...
  g.children(
      {dot(SkPoint{kCx, kCy}, kMaterR, brass(0.50f))
       ...
       dot(SkPoint{kCx, kCy}, kMaterR,
           ...
           .opacity(
               animate(from(0.0f).to(1.0f), ramp(tMater * 1000 + 200, 700))),
       ...
       kit::disc(SkPoint{kCx, kCy}, kMaterR)
           ...
           .opacity(
               animate(from(0.0f).to(1.0f), ramp(tMater * 1000 + 300, 600))),
  ...
  // the throne (kursi) and its shackle — the instrument hangs plumb
  {
    ...
    g.children(
        {box()
             ...
             .opacity(
                 animate(from(0.0f).to(1.0f), ramp(tMater * 1000 + 200, 600))),
