...
auto ChaucerAstrolabe::verify() -> void {
  ...
  A.add(measure::heading("STEREOGRAPHIC — CIRCLES STAY CIRCLES"))
      .add(measure::reading("ecliptic sampled at N longitudes", 3600))
      .add(
          measure::check("max |dist to centre − r_ecl|, R", 0.0, maxEcl, 1e-14))
      .add(measure::reading("almucantar h=30, azimuths fitted", 720))
      .add(measure::check("|centre − closed form|, R", 0.0,
                          std::abs(fa.cy - almCyD(30.0)), 1e-13))
      .add(measure::check("|radius − closed form|, R", 0.0,
                          std::abs(fa.r - almRD(30.0)), 1e-13))
      .add(measure::check("least-squares residual, R", 0.0, fa.res, 1e-13))
      .add(measure::heading("TANGENCY — THE ECLIPTIC TOVCHES BOTH TROPICS"))
      .add(measure::check("|c_ecl| + r_ecl − R_cap", 0.0,
                          std::abs(kEclCyD) + kEclRD - 1.0, 1e-15))
      .add(measure::check("r_ecl − |c_ecl| − R_can", 0.0,
                          kEclRD - std::abs(kEclCyD) - kRcanD, 1e-15))
      .add(measure::heading("THE HORIZON MEETS THE EQVATOR EAST AND WEST"))
      .add(measure::reading("R_eq / sin φ", kReqD / std::sin(kPhiD * kDD)))
      .add(measure::check("dist((R_eq,0) → horizon centre)",
                          kReqD / std::sin(kPhiD * kDD),
                          std::hypot(kReqD, almCyD(0.0)), 1e-14))
      .add(measure::check("prime vertical: R_eq² + cy² "
                          "− a²",
  ...
  B.add(measure::heading("THE AZIMVTH FAMILY, CHECKED THE HARD WAY"))
      ...
      .add(measure::check("A = 0/180 degenerates to a LINE: max |x|", 0.0,
                          lineMax, 1e-15))
      .add(measure::reading("↑ the check that caught it", "1.7 R"));
  ...
  B.add(measure::heading("THE SEASONAL HOVRES — AND THEIR DOCVMENTED ERROVR"))
  ...
  // --- the zodiac cells tile the ring ---------------------------------
  {
    ...
    C.add(measure::heading("THE ZODIAC CELLS TILE THE RING"))
        ...
        .add(measure::check("uncovered − outside-the-ring", 0,
                            cov.uncovered - ref.uncovered))
        .add(measure::check("Σ spans, degrees", 360.0, sum, 1e-3));
  }
  ...
  // --- telling the time -----------------------------------------------
  {
    ...
    const double Ha = std::acos((std::sin(h * kDD) -
                      ...
                      kDD;
    chaucerH =
        kit::formatted("graphical (two DRAWN circles cut)  H = %.9f°", Hg);
    chaucerA = kit::formatted(
        "analytic  (cos H = (sin h − sφ s"
        "δ)/(cφ cδ))   %.9f°",
        Ha);
    chaucerDelta = kit::formatted(
        "agreement  %.2e° — the DRAWN "
        "geometry encodes the trigonometry",
        std::abs(Hg - Ha));
    D.add(measure::heading("TELLING THE TIME — 12 MARCH 1391, "
                           "ALT 25° 30′"))
        ...
        .add(measure::reading("lettre X (21st) · Chaucer read",
                              "9 of the clokke"))
  ...
  // --- the instrument's own error --------------------------------------
  {
    ...
    D.add(measure::heading("THE INSTRVMENT'S OWNE ERROVR"))
        .add(measure::reading("ε Chaucer I.17, R_eq", (double)kReq))
        .add(measure::reading("ε true 1326, R_eq", (double)kt))
        ...
        .add(measure::finding(
            measure::check("ε = 23° 50′ is the true obliquity",
