...
//
// EDIT THESE FIRST
//   ctx.canvas(96, 96) — the asset's exact pixel size, and the number
//                        every dimension below is a fraction of. It is
//                        what --scale 1 captures.
//   ctx.background     — transparent, because a nine-slice frame is laid
//                        over whatever it frames.
//   the paint program  — the art. Everything else on this page is the
//                        export workflow, which is what the template is.
...
struct FrameAsset : sketch::Sketch {
  void setup(sketch::SketchContext& ctx) override {
    ...
    ctx.composer.render(box().inset(0).child(
        custom([](SkCanvas& c, const PaintContext& paint) {
          const float s = paint.size.width();
          SkPaint brush;
          brush.setAntiAlias(true);

          // Parchment center (stretches under content).
          brush.setColor4f({0.88f, 0.80f, 0.62f, 0.96f}, nullptr);
          ...
          c.drawRoundRect(
              SkRect::MakeLTRB(s * 0.07f, s * 0.07f, s * 0.93f, s * 0.93f),
          ...
          const float at[4][2] = {
              {0.13f, 0.13f}, {0.87f, 0.13f}, {0.87f, 0.87f}, {0.13f, 0.87f}};
          for (auto& corner : at) {
            brush.setColor4f({0.22f, 0.13f, 0.07f, 1}, nullptr);
            c.drawCircle(s * corner[0], s * corner[1], s * 0.085f, brush);
            brush.setColor4f({0.85f, 0.64f, 0.22f, 1}, nullptr);
            SkPathBuilder d;
