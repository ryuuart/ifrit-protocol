...
def eligible(nid):
    ...
    return (
        nd
        and not nd.get("ascendancyName")
        and not nd.get("isProxy")
        and nd.get("classStartIndex") is None
        and nd.get("group") is not None
    )
...
inDisc = {
    n
    for n in nodes
    if eligible(n) and pos(nodes[n]) and math.dist(pos(nodes[n]), seedPos) <= RADIUS
}
...
KEYSTONE_TEXT = [
    ...
    (
        "HOLLOW COVENANT",
        ["Life recovery applies to Energy Shield instead", "Maximum Life is halved"],
...
NOTABLE_TEXT = [
    ("Kindled Reserve", ["18% increased Fire Damage", "+12 to maximum Life"]),
    ("Cinderpath", ["20% increased Damage over Time", "10% increased Area of Effect"]),
    ("Ashen Discipline", ["+16% to Fire Resistance", "6% increased maximum Life"]),
    ("Wickwright", ["25% increased Ignite Duration", "12% increased Burning Damage"]),
    (
        "Tinderheart",
    ...
    ("Sootbound", ["30% increased Armour", "5% additional Physical Damage Reduction"]),
    (
        "Emberglass",
        ["+40 to maximum Energy Shield", "10% increased Energy Shield Recharge Rate"],
    ),
]
MINOR_TEXT = [
...
for n in order:
    ...
    state = (
        "State::Allocated"
        if n in allocated
        else "State::CanAllocate"
        if n in frontier
        else "State::Unallocated"
    )
    w(
        '    {%.2ff, %.2ff, %d, %d, Kind::%s, %s, "%s"},'
        % (
            x,
            y,
            group_index[nd["group"]],
            nd.get("orbit", 0),
            k.capitalize(),
            state,
            esc(node_name[n]),
...
for a in order:
    for b in sorted(sub[a]):
        ...
        arc = (
            na["group"] == nb["group"]
            and na.get("orbit", 0) == nb.get("orbit", 0)
...
w("};")
w("inline constexpr int kDetailCount = (int)(sizeof(kDetails) / sizeof(kDetails[0]));")
w("")
w("}  // namespace skill_tree_data")
w("")
