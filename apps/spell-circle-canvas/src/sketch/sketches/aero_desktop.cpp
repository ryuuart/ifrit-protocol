/** @file
 * aero desktop — glass, and the window colorization behind it: the
 * blur, the tint and the edge light that made a decade of desktops.
 */
...
struct AeroDesktop final : sketch::Sketch {
  ...
  Element clientArea() {
    ...
    return box()
        ...
        .child(text("aurora_over_tromso.jpg")
                   .ink(gray(0.15f))
                   .inset(172, 54, 0, 0))
        .child(text("colorization_formula.txt")
                   .ink(gray(0.35f))
                   .inset(172, 82, 0, 0))
        .child(
            text("blurdeviation_30.reg").ink(gray(0.35f)).inset(172, 106, 0, 0))
        .child(text("sky_74B8FC_balances_8_43_49.theme")
                   .ink(gray(0.35f))
                   .inset(172, 130, 0, 0));
  }
