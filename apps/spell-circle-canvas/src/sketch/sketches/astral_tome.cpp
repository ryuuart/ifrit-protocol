...
//         GlStateManager.translate(offsetX + (width / 2), offsetY + (width /
//         2), …)
...
//   CONSTELLATIONS_PER_PAGE = 4               Cluster:54  (the offsetMap
//   comment width = 80, height = 110                  Cluster:59   says "we put
//   6 on '1' offsetMap 0..3 = (45,55) (125,105)        Cluster:288
//   page/screen". Four
...
//   bookmark labels                           ClientProxy:196-205  when
//   unselected
//
...
//   brightness      0.3 + 0.7 * conCFlicker(tick, partial, 12 +
//   rand.nextInt(10))
//                   conCFlicker = sin(((t % (dayLength/2)) + partial)/d)/4 +
//                   0.375 dayLength = 24000 (Config:96), so the modulus is
//                   12000 ticks
...
//   * a brush::Ribbon body under the rails whose width Profile takes the band
//   to
...
// out THICKER. lines::Rails shares the property — geometry::path::parallel and
// the rail widths are local-space too — so the whole rail vocabulary is
// anisotropic under a non-uniform parent scale.
//
    ...
    for (int i = 0; i < 6; ++i) {
      ...
      Element art =
          box()
              ...
              .fill(Fill::color(at::scaleRgb(at::kFieldStar, 1.0f, r.alpha)));
      p.child(box()
                  .inset(-60)
  ...
  Element linkPass(const at::Con& c, int li, int pass, int key) const {
    ...
    rails.rails = {
        {.across = 0,
         ...
         .fill = Fill::color(at::scaleRgb(col, 1.0f, 0.309f))},
        {.across = 0,
         .width = band * (9.0f / 64.0f),
         .fill = Fill::color(at::scaleRgb(col, 1.45f, 0.580f))},
        ...
        {.across = -half * 1.9f,
         .width = 1.4f,
         .fill = Fill::color(at::scaleRgb(col, 1.35f, 0.52f)),
         .dash = {2.2f, 9.4f},
    ...
    brush::Ribbon bloom;
    bloom.fill = Fill::color(at::scaleRgb(col, 1.0f, 0.055f));
    bloom.step = 6.0f;
    bloom.width = at::LinkTaper{band * 2.1f};

    brush::Ribbon body;
    body.fill = Fill::color(at::scaleRgb(col, 1.0f, 0.135f));
    body.step = 4.0f;
  ...
  Element starEl(const at::Con& c, int si, int key) const {
    ...
    grp.child(
        box()
            ...
            .fill(Fill::color(at::scaleRgb(col, 1.15f, 0.74f))));
    ...
    Element e =
        box()
            ...
            .foreground(decorations::border(
                1.2f, Fill::color(at::scaleRgb(at::kGilt, 1.0f, 0.7f))));
    if (hovered)
      e.scale(1.1f);
    else
  ...
  void setup(sketch::SketchContext& ctx) override {
    ...
    for (int ci = 0; ci < 4; ++ci) {
      ...
      root.child(label(c.name, at::gx(o.fX + at::kCellW * 0.5f) - w * 0.5f,
                       ...
                       at::scaleRgb(at::kInk, 1.0f, kInkAlphaOf()), 2.4f)
