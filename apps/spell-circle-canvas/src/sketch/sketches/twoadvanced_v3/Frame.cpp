  ...
  return box()
      ...
      .foreground(
          onEdges(path::Edge::Bottom,
                  stroke(1, Fill::color(sigil::material::withAlpha(kInk, 0.6f)),
                         PathFormat::Align::Inner)))
      .foreground(onEdges(
          path::Edge::Top,
          stroke(1, Fill::color(sigil::material::withAlpha(kSteelHi, 0.7f)),
                 PathFormat::Align::Inner)))
      .children(
          {kit::centred()
               .width(16)
           ...
           box().width(4).height(4).fill(
               sigil::material::withAlpha(kInk, 0.8f)),
           box().width(4).height(4).fill(
               sigil::material::withAlpha(kInk, 0.5f)),
           box().width(4).height(4).fill(
               sigil::material::withAlpha(kInk, 0.3f))});
}
...
Element TwoAdvancedV3::meter(int lit) {
  ...
  for (int i = 0; i < 5; ++i)
    m.children({box().width(9).height(7).fill(
        i < lit ? sigil::material::withAlpha(kSteelHi, 0.9f)
                : sigil::material::withAlpha(kSteelDim, 0.4f))});
  return m;
}
  ...
  Element panel =
      at(box().row().alignItems(Align::Center).padding(0, 30).gap(16), kStageX,
         82, kStageW, 86)
          ...
          .foreground(onEdges(
              path::Edge::Bottom,
              stroke(2, Fill::color(sigil::material::withAlpha(kInk, 0.5f)),
                     PathFormat::Align::Inner)))
          .children(
              {mark,
...
Element TwoAdvancedV3::navBar() {
  ...
  bar.children(
      {box()
           ...
           .padding(0, 12)
           .gap(7)
           .fill(sigil::material::withAlpha(hexColor(0x39445C), 0.92f))
           .foreground(onEdges(
               path::Edge::Right,
               stroke(1, Fill::color(sigil::material::withAlpha(kInk, 0.8f)),
                      PathFormat::Align::Inner)))
    ...
    row.children(
        {kit::centred()
             ...
             .children(
                 {t(kSections[i].tab,
                    micro(13.5f,
                          on ? kNear : sigil::material::withAlpha(kNear, 0.88f),
                          170)),
                  box().width(46).height(2).fill(
                      on ? sigil::material::withAlpha(kSteelHi, 0.95f)
...
Element TwoAdvancedV3::hairlines() {
  ...
  return at(box().column().gap(2).children(
                {box().height(2).fill(
                 ...
                 box()
                     .height(3)
                     .fill(sigil::material::withAlpha(kSeam, 0.95f))
                     .children({box()
                                    .inset(0)
                ...
                .foreground(
                    onEdges(path::Edge::Top,
                            stroke(1,
                                   ...
                                   PathFormat::Align::Inner)))
                .children(
                    {t(strip["arrow"], micro(9, kSteelHi, 0)),
                     t(strip["left"],
                       micro(9, sigil::material::withAlpha(kSteelHi, 0.85f),
                             180)),
                     box().flexGrow(1),
                     t(strip["right"],
...
Element TwoAdvancedV3::bootOverlay() {
  ...
  const sigil::material::Color kPreBg = hexColor(0x2A3753),
                               kPreInk = hexColor(0x7183A5);
