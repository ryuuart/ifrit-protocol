...
//
// NO CURVED LETTERING, DELIBERATELY. Text::textOnPath() would be the obvious
...
struct ChladniTab1 {
    ...
    root.children(
        {box().inset(0).children(
         ...
         text("Capieux. sculps. 1786.")
             ...
             .ink(kInkSoft)
             .key("credit")
             .centerAt({kFrameR - 154.0f, kFrameB - 32.0f})
             .opacity(
  ...
  void setup(sketch::SketchContext& ctx) {
    ...
    foxing.seed(17);
    foxingLL =
        patterns::speckle(520, 14, 2.0f, 7.0f, {hexColor(0x94764c, 0.09f)});
    foxingLL.seed(53);
