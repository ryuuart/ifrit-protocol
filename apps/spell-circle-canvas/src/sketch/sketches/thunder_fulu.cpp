...
//   geometry::path::displace      雲篆: the cloud-wander is a REAL median
//   displaced,
...
struct ThunderFulu : sketch::Sketch {
  ...
  void buildStrokes() {
    ...
    // 0.034 s a stroke against the body's 0.240.
    {
      ...
      const float xs[5] = {kCol - 18, kCol + 24, kCol - 22, kCol + 20,
                           kCol - 4};
      SkPathBuilder b;
      std::vector<std::array<float, 3>> spans;
  ...
  Element plate() {
    ...
    const Reg regs[5] = {
        {34, "FU TOU  head  \xc2\xb7 3 hooks, 3 Pure Ones"},
  ...
  Element chantPanel() {
    ...
    for (int i = 0; i < 5; ++i) {
      ...
      g.child(
          text(toU8(lines_[i]), type(faceMono, 11.5f, kChalk))
              .left(0)
              .top(36 + (float)i * 19)
              .width(468)
  ...
  Element marginColumn() {
    ...
    for (int k = 0; k < 3; ++k) {
      ...
      g.child(
          text(toU8(kHeadChant[k]), type(faceItalic, 11.0f, kChalk))
              .left(0)
              .top(154 + (float)k * 26)
              .width(Dim(Wc))
  ...
  Element furniture() {
    ...
    // title block
    g.child(
        text(toU8("WU LEI HAO LING \xc2\xb7 A THUNDER-RITE COMMAND TALISMAN, "
                  "WRITTEN"),
            ...
            .width(1400));
    g.child(
        text(toU8("DAOFA HUIYUAN DZ 1220, juan 46 \xc2\xb7 iron plate, five "
