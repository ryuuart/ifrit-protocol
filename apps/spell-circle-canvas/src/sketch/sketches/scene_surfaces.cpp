...
/** THE ROOT EVERY SCREEN IS: the whole texture, the ground it is painted
...
compose::Element levels(float seconds, SkColor4f accent) {
  ...
  const auto bar = [seconds, accent](int i) {
    const float height =
        18.0f +
        62.0f * (0.5f + 0.5f * std::sin(seconds * 2.1f + (float)i * 0.55f));
    return compose::box().width(14.0f).children(
...
compose::Element band(float seconds) {
  ...
  const auto cell = [seconds](int i) {
    const float lit =
        0.12f + 0.88f * std::pow(0.5f + 0.5f * std::sin(seconds * 1.4f -
                                                        (float)i * 0.34f),
                                 4.0f);
    return compose::box()
        .width(compose::pct(100))
