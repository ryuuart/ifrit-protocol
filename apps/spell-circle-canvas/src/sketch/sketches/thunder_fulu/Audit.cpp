...
auto ThunderFulu::runChecks() -> void {
  ...
  logA.append({toU8(kit::formatted("  GANG U+7F61 has %d strokes",
                                   font.glyphs[GANG].strokes)),
               stems});
  logA.append({toU8("  doctrine: 10 strokes = the ten Heavenly Stems"), "dim"});
  ...
  logB.append(
      {toU8("  GUI U+9B3C: mmah 9, KanjiVG 10 \xe2\x80\x94 the Japanese"),
       "dim"});
  logB.append({toU8("    form carries an extra PIE; and the two sources order"),
               "dim"});
  logB.append(
  ...
  logC.append(
      {toU8("  comparable Profile. BUT under trim() a decoration is"), "dim"});
  logC.append(
      {toU8("  handed the REVEALED contour: a fraction is a fraction"), "dim"});
  logC.append({toU8("  of what is drawn SO FAR, so the profile is keyed in PX"),
               "dim"});
