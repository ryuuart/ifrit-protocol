...
auto ThunderFulu::runChecks() -> void {
  ...
  logA.append(
      {"makemeahanzi graphics.txt — MEDIANS, 1024 em, "
       "DRAWING ORDER",
       "heading"});
  ...
  logA.append({kit::formatted("  11 characters, %d strokes, %d median points, "
                              "verbatim",
                              totalStrokes, totalPts),
               "dim"});
  const char* stems = claim("GANG U+7F61 strokes, the ten Heavenly Stems", 10,
                            font.glyphs[GANG].strokes);
  logA.append({kit::formatted("  GANG U+7F61 has %d strokes",
                              font.glyphs[GANG].strokes),
               stems});
  logA.append({"  doctrine: 10 strokes = the ten Heavenly Stems", "dim"});
  logA.append(
      {"  JIA YI BING DING WU JI GENG XIN REN GUI — "
       "the count MATCHES",
       stems});
  logA.append(
      {kit::formatted("  foot JI+JI+RU+LU+LING = %d+%d+%d+%d+%d = %d strokes",
  ...
  logA.append(
      {kit::formatted("    drawn as ONE contour: %d spans = %d strokes + "
                      "%d ligatures",
  ...
  logA.append({kit::formatted("  ink strokes on the plate: %d nodes",
                              (int)strokes.size()),
               "number"});

  // --- panel B: the taxonomy --------------------------------------------
  logB.append({"KanjiVG kvg:type — THE STROKE CLASS, FOR w0", "heading"});
  ...
  logB.append(
      {"  kanji/07f61.svg (GANG) is a 14-byte 404. NOT A KANJI.", "fail"});
  logB.append({"  recovered instead from the median geometry: length,", "dim"});
  logB.append({"  body chord angle over 14-86%, peak turn from entry", "dim"});
  logB.append(
      {kit::formatted("  vs KanjiVG over the %d strokes it does carry: %d/%d",
  ...
  for (size_t i = 0; i < kvgMiss.size() && i < 3; ++i)
    logB.append({"    " + kvgMiss[i], "dim"});
  logB.append({"  GUI U+9B3C: mmah 9, KanjiVG 10 — the Japanese", "dim"});
  logB.append(
      {"    form carries an extra PIE; and the two sources order", "dim"});
  logB.append({"    TIAN's interior in LEI U+96F7 opposite ways", "number"});
  {
    ...
    logB.append({row, "number"});
  }

  // --- panel C: the brush ------------------------------------------------
  logC.append(
      {"THE BRUSH · QI / XING / SHOU, one law over arc "
       "length",
       "heading"});
  logC.append(
      {"  w(s)/w0 = 1.15e^-12s + 0.62 + 0.28s + 0.55e^-90(s-.88)^2", "dim"});
  logC.append({kit::formatted("    ni feng (reverse entry) w(0)    = %.2f w0",
               ...
               "number"});
  {
    ...
    logC.append({kit::formatted("    zhong feng belly  min %.2f w0 at s = %.2f",
                                (double)minW, (double)minS),
                 "number"});
  }
  logC.append({kit::formatted("    dun (the press)   w(0.88) = %.2f w0",
                              (double)widthLaw(0.88f)),
               "number"});
  logC.append({kit::formatted("    shou (the cut)    w(1)    = %.2f w0",
                              (double)widthLaw(1.0f)),
               "number"});
  logC.append({"  brush::Ribbon::width takes exactly this law, as a", "dim"});
  logC.append(
      {"  comparable Profile. BUT under trim() a decoration is", "dim"});
  logC.append(
      {"  handed the REVEALED contour: a fraction is a fraction", "dim"});
  logC.append(
      {"  of what is drawn SO FAR, so the profile is keyed in PX", "dim"});
  logC.append(
      {"  or the dun press SLIDES down the stroke as it writes.", "pass"});
  logC.append(
      {kit::formatted("  tempo: foot %.3f s/stroke vs body %.3f = %.1fx",
