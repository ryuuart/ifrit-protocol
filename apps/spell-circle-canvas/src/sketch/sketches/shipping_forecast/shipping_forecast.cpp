...
//
// EDIT THESE FIRST
//   data/content.json — the bulletin: the sixteen sea areas and the compass
//                bearing each is set at (move a bearing and the name moves
//                round the ring), the forecast's and the synopsis's runs,
//                the readouts, the stations and every line of furniture.
...
struct ShippingForecast {
  ...
  [[nodiscard]] sigil::weave::Type voice() const {
    return {.face = faceBody, .size = 19.5f, .color = kBone};
  }
  ...
  [[nodiscard]] sigil::compose::StyleSheet registers() const {
    return sigil::compose::StyleSheet{
        ...
        sigil::compose::rule(".dir").font(
            {.face = faceBold, .track = 0.6f, .condense = 0.94f}),
        ...
        sigil::compose::rule(".force").font(
            {.face = faceBold, .size = 10.5f, .track = 0.4f}),
        ...
        sigil::compose::rule(".hero").font({.face = faceDisplay,
                                            .size = kHero,
                                            .color = kBone,
                                            .track = 1.5f}),
        ...
        // The small print: a note under a readout, a station's name and its
        // wind, the Beaufort bands, the spine, the foot.
        sigil::compose::rule(".note").font(
            {.size = 12.0f, .color = kSlateDim, .track = 0.6f}),
        sigil::compose::rule(".place").font(
            {.size = 12.5f, .color = kBone, .track = 0.8f}),
        sigil::compose::rule(".wind").font(
            {.face = faceBold, .size = 12.5f, .color = kSlate, .track = 1.4f}),
        sigil::compose::rule(".bands").font(
            {.size = 10.5f, .color = kSlateDim, .track = 0.8f}),
        sigil::compose::rule(".spine").font({.face = faceBold,
                                             .size = 12.5f,
                                             .color = kSlateDim,
                                             .track = 2.6f}),
        sigil::compose::rule(".foot").font(
            {.size = 11.0f, .color = kSlateDim, .track = 0.5f})};
  }
  ...
  [[nodiscard]] Element forecast() {
    ...
    const weave::Selector glossary = selectors::style("term");
    ...
    return box().column().gap(9).children({
        ...
        document::paragraph(copy)
            ...
            .span(weave::selectors::regex(u8"[0-9]+"),
                  SpanStyle().ink(kAmber).font({.color8 = true}))
  ...
  [[nodiscard]] Element synopsis() {
    ...
    return box().column().gap(9).children({
        ...
        document::paragraph(copy)
            ...
            .paragraph(
                {.lineBreak = sigil::weave::LineBreakStrategy::kKnuthPlass})
            .span(weave::selectors::regex(u8"[0-9]+"), SpanStyle().font(graded))
            .span(weave::selectors::regex(u8"[0-9]+"),
                  SpanStyle().ink(kAmber).font({.color8 = true}))
