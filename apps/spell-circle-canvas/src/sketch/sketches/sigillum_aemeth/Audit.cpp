...
auto SigillumAemeth::runChecks() -> void {
  ...
  logA.append({toUtf8("SECUNDUS.PDF p.31 \xe2\x80\x94 THE PLATE AS VECTORS"),
               "heading"});
  logA.append({toUtf8("  310 text objects; 296 after the colophon"), "dim"});
  logA.append(
      {toUtf8("  K\xc3\xa5sa fit, outer letter ring, 44 glyphs:"), "dim"});
  logA.append(
      {toUtf8("    centre (305.185, 393.529) pt   R = 257.972 pt"), "number"});
  logA.append(
      {toUtf8("    page bbox centre is off by (-0.82, -2.47) pt"), "dim"});
  logA.append({toUtf8("    residual rms 4.35 pt = 1.7% R"), "dim"});
  logA.append(
      {toUtf8("  9\xc2\xb0 lattice fit: phase +4.30\xc2\xb0, rms 0.81\xc2\xb0"),
       "number"});
  logA.append({toUtf8("  40 cells recovered: 40 letters, 33 numerals"), "dim"});
  logA.append(
      {toUtf8("  above/below decided by r(numeral) vs r(letter)"), "dim"});
  {
    int above = 0, below = 0, bare = 0;
    for (const Cell& c : kRing)
      (c.step > 0 ? above : c.step < 0 ? below : bare)++;
    logA.append({toUtf8(kit::formatted("    %d above  %d below  %d bare   = 40",
                                       above, below, bare)),
  ...
  // --- panel B: the walk -------------------------------------------------
  logB.append(
      {toUtf8("MICHAEL'S JUMP RULE, WALKED FROM ALL 40 CELLS"), "heading"});
  for (int n = 0; n < 7; ++n) {
    ...
    logB.append({toUtf8(kit::formatted("  %-9s %-9s %s", kNames[(size_t)n].name,
                                       s.raw.c_str(), chain.c_str())),
                 ok ? "pass" : "heading"});
  }
  logB.append({toUtf8(kit::formatted("  cells consumed %d of 40, unvisited %d",
               ...
               usedCells == 33 ? "pass" : "heading"});
  {
    ...
    logB.append({toUtf8("  " + un), "number"});
  }
  logB.append(
      {toUtf8("  Aaoth from 27 also spells Aaoth \xe2\x80\x94 but leaves"),
       "dim"});
  logB.append({toUtf8("  32 cells. The 33/7 count picks cell 32."), "pass"});
  ...
  // --- panel C: coverage -------------------------------------------------
  {
    ...
    logC.append({toUtf8("THE 40 CELLS TILE THE ANNULUS"), "heading"});
    logC.append({toUtf8(kit::formatted("  doubled %d of %d samples",
                                       cov.doubled, cov.samples)),
                 cov.doubled == 0 ? "pass" : "heading"});
    logC.append(
        {toUtf8(kit::formatted("  uncovered %d - outside-the-ring %d = %d",
  ...
  }
  {
    ...
    const test::Coverage ref =
        test::coverage(std::span<const SkPath>(&ring, 1), reg, 320);
    logC.append({toUtf8("THE 7 ANGLE PLATES TILE THEIR BAND"), "heading"});
    logC.append(
        {toUtf8(kit::formatted("  doubled %d, gap vs ring %d", cov.doubled,
                               cov.uncovered - ref.uncovered)),
         cov.doubled == 0 ? "pass" : "heading"});
    int letters = 0;
    ...
    logC.append({toUtf8(kit::formatted("  %d letters + 1 cross = %d places",
                                       letters, letters + 1)),
    ...
    logC.append(
        {toUtf8("  down the columns: " + cols.substr(0, 24)), "number"});
    logC.append({toUtf8("  " + cols.substr(24)), "number"});
  }

  // --- panel D: {7/2} vs {7/3}, and the weave ---------------------------
  logD.append({toUtf8("{7/2} OR {7/3}? THE COORDINATES DECIDE"), "heading"});
  logD.append(
      {toUtf8(kit::formatted("  {7/2} core = %.4f x Rhept = %.3f R",
                             (double)kStar72, (double)(kStar72 * rHept))),
       "number"});
  logD.append(
      {toUtf8(kit::formatted("  {7/3} core = %.4f x Rhept = %.3f R",
                             (double)kStar73, (double)(kStar73 * rHept))),
       "number"});
  logD.append({toUtf8(kit::formatted(
  ...
  logD.append(
      {toUtf8(kit::formatted("  Filiae/Filii Filiorum measured %.3f / %.3f R",
                             (double)rFiliaeFil, (double)rFiliiFil)),
       "dim"});
  logD.append(
      {toUtf8("  both INSIDE a {7/2} core, as the record says;"), "dim"});
  logD.append(
      {toUtf8("  {7/3} would leave them floating mid-point and"), "dim"});
  logD.append(
      {toUtf8("  would collide with ZABATHIEL at 0.285 R.  {7/2}."), "pass"});
  logD.append({toUtf8("THE INTERLACE"), "heading"});
  ...
  test::report(
      ...
      "pass", "heading", 30, 4);
  logD.append(
      {toUtf8("  gcd(40,7)=gcd(40,5)=gcd(7,5)=1 \xe2\x80\x94 one"), "dim"});
  logD.append({toUtf8("  alignment only, and it is 12 o'clock."), "pass"});
}
