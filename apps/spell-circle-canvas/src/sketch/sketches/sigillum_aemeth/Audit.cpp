...
auto SigillumAemeth::runChecks() -> void {
  ...
  logA.append({"SECUNDUS.PDF p.31 — THE PLATE AS VECTORS", "heading"});
  logA.append({"  310 text objects; 296 after the colophon", "dim"});
  logA.append({"  Kåsa fit, outer letter ring, 44 glyphs:", "dim"});
  logA.append({"    centre (305.185, 393.529) pt   R = 257.972 pt", "number"});
  logA.append({"    page bbox centre is off by (-0.82, -2.47) pt", "dim"});
  logA.append({"    residual rms 4.35 pt = 1.7% R", "dim"});
  logA.append({"  9° lattice fit: phase +4.30°, rms 0.81°", "number"});
  logA.append({"  40 cells recovered: 40 letters, 33 numerals", "dim"});
  logA.append({"  above/below decided by r(numeral) vs r(letter)", "dim"});
  {
    int above = 0, below = 0, bare = 0;
    for (const Cell& c : kRing)
      (c.step > 0 ? above : c.step < 0 ? below : bare)++;
    logA.append({kit::formatted("    %d above  %d below  %d bare   = 40", above,
                                below, bare),
                 above + below + bare == 40 ? "pass" : "heading"});
  }

  // --- panel B: the walk -------------------------------------------------
  logB.append({"MICHAEL'S JUMP RULE, WALKED FROM ALL 40 CELLS", "heading"});
  for (int n = 0; n < 7; ++n) {
    ...
    logB.append({kit::formatted("  %-9s %-9s %s", kNames[(size_t)n].name,
                                s.raw.c_str(), chain.c_str()),
                 ok ? "pass" : "heading"});
  }
  logB.append({kit::formatted("  cells consumed %d of 40, unvisited %d",
               ...
               usedCells == 33 ? "pass" : "heading"});
  {
    ...
    logB.append({"  " + un, "number"});
  }
  logB.append({"  Aaoth from 27 also spells Aaoth — but leaves", "dim"});
  logB.append({"  32 cells. The 33/7 count picks cell 32.", "pass"});
  ...
  // --- panel C: coverage -------------------------------------------------
  {
    ...
    logC.append({"THE 40 CELLS TILE THE ANNULUS", "heading"});
    logC.append(
        {kit::formatted("  doubled %d of %d samples", cov.doubled, cov.samples),
         cov.doubled == 0 ? "pass" : "heading"});
    logC.append(
        {kit::formatted("  uncovered %d - outside-the-ring %d = %d",
  ...
  }
  {
    ...
    const test::Coverage ref =
        test::coverage(std::span<const SkPath>(&ring, 1), reg, 320);
    logC.append({"THE 7 ANGLE PLATES TILE THEIR BAND", "heading"});
    logC.append({kit::formatted("  doubled %d, gap vs ring %d", cov.doubled,
                                cov.uncovered - ref.uncovered),
                 cov.doubled == 0 ? "pass" : "heading"});
    int letters = 0;
    for (const auto& angleRow : kAngles)
      for (const auto& cell : angleRow)
        if (cell != std::string("†")) ++letters;
    logC.append({kit::formatted("  %d letters + 1 cross = %d places", letters,
                                letters + 1),
                 letters == 48 ? "pass" : "heading"});
    std::string cols;
    for (int c = 0; c < 7; ++c)
      for (const auto& angleRow : kAngles)
        if (angleRow[c] != std::string("†")) cols += angleRow[c];
    logC.append({"  down the columns: " + cols.substr(0, 24), "number"});
    logC.append({"  " + cols.substr(24), "number"});
  }

  // --- panel D: {7/2} vs {7/3}, and the weave ---------------------------
  logD.append({"{7/2} OR {7/3}? THE COORDINATES DECIDE", "heading"});
  logD.append({kit::formatted("  {7/2} core = %.4f x Rhept = %.3f R",
                              (double)kStar72, (double)(kStar72 * rHept)),
               "number"});
  logD.append({kit::formatted("  {7/3} core = %.4f x Rhept = %.3f R",
                              (double)kStar73, (double)(kStar73 * rHept)),
               "number"});
  logD.append(
      {kit::formatted("  along a point's ray the core stops at %.3f R",
                      (double)(kStar72 * rHept * std::cos(3.14159265f / 7))),
       "dim"});
  // "Filiae", not "Filiæ": the feed runs in the MONO face, which
  ...
  logD.append({kit::formatted("  Filiae/Filii Filiorum measured %.3f / %.3f R",
                              (double)rFiliaeFil, (double)rFiliiFil),
               "dim"});
  logD.append({"  both INSIDE a {7/2} core, as the record says;", "dim"});
  logD.append({"  {7/3} would leave them floating mid-point and", "dim"});
  logD.append({"  would collide with ZABATHIEL at 0.285 R.  {7/2}.", "pass"});
  logD.append({"THE INTERLACE", "heading"});
  ...
  test::report(
      ...
      "pass", "heading", 30, 4);
  logD.append({"  gcd(40,7)=gcd(40,5)=gcd(7,5)=1 — one", "dim"});
  logD.append({"  alignment only, and it is 12 o'clock.", "pass"});
}
