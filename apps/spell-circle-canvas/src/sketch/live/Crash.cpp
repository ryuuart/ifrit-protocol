/** @file
 * The crash reporter's handlers: what the host was doing, which frame it
...
void emitNumber(long value) {
  ...
  unsigned long magnitude =
      negative ? (unsigned long)(-value) : (unsigned long)value;
...
const char* phaseName(int phase) {
  switch ((Phase)phase) {
    ...
    case Phase::Draw:
      return "Composer::draw() — painting the described tree";
    case Phase::Capture:
      return "capture (readback + PNG encode)";
    case Phase::Host:
      break;
  }
...
const char* signalName(int number) {
  switch (number) {
    ...
    case SIGBUS:
      return "SIGBUS (bad address / alignment)";
    case SIGILL:
      return "SIGILL (illegal instruction — often a bad vtable "
             "or a pointer-authentication failure)";
    case SIGFPE:
      return "SIGFPE (arithmetic fault)";
    case SIGABRT:
      return "SIGABRT (abort — an uncaught exception reaches here, "
