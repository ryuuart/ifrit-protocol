...
def component(model: Reading) -> Element:
    return column(
        ...
        box(height=8, width="100%", fill=wash("#356c69"), scale_x=model.level),
        None,
        ["A native element", row(text("with ordinary children"))],
    ...
    def setup(self, ctx: SketchContext) -> None:
        ...
        with kit.provide(look):
            body = row(
                ...
                native,
                graphics(paint, key="mark", width=24, height=24),
...
assert_type(layout(Grid(columns=[fr(), fr()]), text("Left"), text("Right")), Element)
