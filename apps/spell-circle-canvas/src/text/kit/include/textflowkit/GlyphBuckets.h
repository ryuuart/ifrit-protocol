 ...
 * of drawGlyphs calls.
 *
 * textflow::GlyphRSXformBatches is the fixed (font, color)+RSXform special
...
inline void drawPositionedGlyphs(SkCanvas *canvas,
                                 const textflow::ShapedWord &font,
  ...
  canvas->drawGlyphs(SkSpan<const SkGlyphID>(glyphs.data(), glyphs.size()),
                     ...
                     origin, textflow::makeFont(font.typeface, font.fontSize),
                     paint);
}
