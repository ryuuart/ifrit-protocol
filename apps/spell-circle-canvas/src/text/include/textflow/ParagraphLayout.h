...
struct OverflowOptions {
  // Empty disables the marker. Only straight horizontal flows can render an
  // ellipsis; curved and vertical flows still report overflow normally.
  std::u16string ellipsis;
};

/** Rendering-only controls that do not affect line breaking. */
struct PathTextOptions {
  // Animated path tangents snap to this many directions to avoid creating a
  // fresh glyph-atlas strike for every tiny rotation change. Zero preserves
  // exact rotations for static artwork.
...
// caller should draw its inline object (see ParagraphLayout::placeholderRects).
...
struct ParagraphLayout {
  ...
  // An overflow marker from ParagraphLayoutOptions::overflow was appended to
  // the final placed line. Its run is the last in `runs`.
