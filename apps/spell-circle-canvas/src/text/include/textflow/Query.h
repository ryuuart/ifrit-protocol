...
// what the layout actually placed (e.g. up to
// ParagraphLayout::firstUnplacedWord's textBegin) and the search is bounded by
// the geometry, not the text.
[[nodiscard]] std::vector<CharRange>
findAllOccurrences(const Paragraph &paragraph, std::u16string_view needle,
                   CharRange scope);
...
/** Returns content-only ranges for the paragraph's analyzed words. */
