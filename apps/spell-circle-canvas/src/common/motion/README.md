...
| `SigilMotionPhysics` | `physics/Points.h`, `physics/Forces.h`, `physics/Constraints.h`, `physics/Verlet.h`, `physics/Particles.h`; `physics/Physics.h` includes all five | `Vec2` and `Points`, the lanes a simulation is; `Force` with `gravity()`, `drag()`, `attract()`/`repel()`, `wind()` and `boids()`; `Constraint` with `distance()`, `stick()`, `spring()`, `range()` and `pin()`; `Verlet`, the stepper; `Particles` and `Attribute`, a point set that is born, ages and dies, with `Emitter`, `EmitFrom`, `Roughly`, `BirthAttribute` and `FixedAttribute`, what puts particles into one |
