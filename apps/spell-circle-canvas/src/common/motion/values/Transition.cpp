...
Curve cubicBezier(float x1, float y1, float x2, float y2) {
  return {[](float t, const float* p) {
            ...
            // One coordinate of the cubic through (0,0), (a,·), (b,·),
            // (1,1).
            ...
            // one whose x is the time asked for, which is exact to far
            // below a pixel at any size a screen has.
            float lo = 0.0f, hi = 1.0f, u = t;
            for (int i = 0; i < 24; ++i) {
              u = 0.5f * (lo + hi);
              (axis(u, p[0], p[2]) < t ? lo : hi) = u;
            }
            return axis(u, p[1], p[3]);
          },
          {x1, y1, x2, y2}};
}
