...
struct Attrs {
  ...
  std::vector<glm::vec4> &ensure(const std::string &name,
    ...
    if (inserted)
      it->second.assign(count, fill);
    return it->second;
  }
  glm::vec4 defaultFor(const std::string &name) const {
    if (name == "Scale" || name == "Color")
      return {1, 1, 1, 1};
    if (name == "Tex")
      return {0, 0, 1, 1};
    if (name == "Dir")
...
Cloud cook(const pop::Chain &chain) {
  ...
  if (scatter && (scatter->loop.size() < 3 || scatter->count < 1))
    return out;
  ...
  const size_t count =
      (size_t)(scatter ? scatter->count : surface->count);
  Attrs attrs;
  attrs.count = count;
  ...
  for (size_t i = 0; scatter && i < count; ++i) {
    ...
    const float ang =
        hash1((uint32_t)i * 7u + seed + 2u) * 6.2831853f;
    const float rad =
        std::sqrt(hash1((uint32_t)i * 5u + seed + 3u)) *
        scatter->radius;
          ...
          if constexpr (std::is_same_v<T, pop::SplineScatter> ||
            ...
            for (size_t i = 0; i < count; ++i) {
              const int cell = std::min(
                  (int)(hash1((uint32_t)i * 13u + op.seed) *
                        (float)cells),
                  cells - 1);
              attrs.store("Tex", i,
                          {(float)(cell % cols) * du,
                           (float)(cell / cols) * dv, du, dv});
            }
          } else {
            for (size_t i = 0; i < count; ++i) {
              if constexpr (std::is_same_v<T, pop::Jitter>) {
                glm::vec4 v = attrs.load(op.lane.name, i);
                v.x += (hash1((uint32_t)i * 3u + op.seed) - 0.5f) *
                       2.0f * op.amplitude;
                v.y += (hash1((uint32_t)i * 5u + op.seed + 1u) -
                        0.5f) *
                       2.0f * op.amplitude;
                v.z += (hash1((uint32_t)i * 7u + op.seed + 2u) -
                        0.5f) *
                       2.0f * op.amplitude;
                attrs.store(op.lane.name, i, v);
              } else if constexpr (std::is_same_v<T, pop::Noise>) {
                const glm::vec3 dd =
                    drift(attrs.p3(i), op.frequency, op.seed) *
                    op.amplitude;
                glm::vec4 v = attrs.load(op.lane.name, i);
                v.x += dd.x;
                v.y += dd.y;
                ...
                const float v =
                    ...
                    (1.0f +
                     op.spread *
                         (hash1((uint32_t)i * 11u + op.seed) * 2.0f -
                          1.0f));
  ...
  for (const auto &[name, lane] : attrs.lanes) {
    if (name == "P" || name == "T" || name == "Dir" ||
        name == "Scale" || name == "Color")
      continue;
...
Mesh cookTube(const pop::Chain &chain, float radius, int sides,
  ...
  if (path.points.size() < 2)
    return {};
  return curves::tube(path, {.radius = radius,
...
Mesh cookRibbon(const pop::Chain &chain, float width,
  ...
  if (path.points.size() < 2)
    return {};
  return curves::ribbon(path,
                        {.width = width, .segments = options.segments});
}
...
Mesh cookSweep(const pop::Chain &chain, const SkPath &profile,
  ...
  if (contours.empty() || contours[0].points.size() < 3)
    return {};
  ...
  for (const Frame3 &f : rail)
    for (uint32_t i = 0; i < n; ++i) {
      ...
      out.positions.push_back(f.position + f.binormal * p.fX -
                              f.normal * p.fY);
