...
/** The tiny safety vocabulary over glm the library's TUs share.
 ...
 *  Pop.cpp's scatter basis is NOT here on purpose: its math is
 *  bit-matched to the GPU kernel and stays literal at the site. */
namespace sigil::shape::detail {

inline glm::vec3 normalized(glm::vec3 v,
 ...
 *  or GPU-drawn. */
inline void basisFor(glm::vec3 dir, glm::vec3 up, glm::vec3 *x,
                     glm::vec3 *y, glm::vec3 *z) {
  *z = normalized(dir);
  glm::vec3 side = glm::cross(up, *z);
  if (glm::dot(side, side) < 1e-8f)
    side = glm::cross(glm::vec3{1, 0, 0}, *z);
  *x = normalized(side);
  *y = glm::cross(*z, *x);
}
