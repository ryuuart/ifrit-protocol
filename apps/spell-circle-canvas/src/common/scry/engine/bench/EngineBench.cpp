/** @file
 * scry_engine_bench — what each compositing direction costs per frame,
 * so scene-render budgets can be planned. One Ultralight renderer per
 * process, so CPU and GPU modes are separate runs of the same binary:
 * `scry_engine_bench` for the CPU engine (raster SkImage frames) and
 * `scry_engine_bench --gpu` for the GPU engine (Metal driver, texture
 * frames); benchmarks that only exist in one mode skip themselves in
 * the other. The draw arms isolate the CONSUMER side: they composite a
 * frame that is already published, so nothing they time depends on how
 * often Ultralight chooses to repaint. Producer-side cost sits in one
 * arm of its own, BM_Page_ChangeLatency, which times a scripted DOM
 * change until the new frame is published and therefore includes the
 * engine's own pacing. Run a Release build; Debug numbers say nothing.
 */
...
#ifdef __APPLE__
  // The GPU-mode arms exist only in a GPU run, so a CPU run (the one the
  // ledger takes) reports nothing for them rather than a skipped zero.
  if (g_useGpu) {
    benchmark::RegisterBenchmark("BM_Frame_WrapCached", BM_Frame_WrapCached);
    benchmark::RegisterBenchmark("BM_Frame_WrapMiss", BM_Frame_WrapMiss);
    benchmark::RegisterBenchmark("BM_Draw_GraphiteRecord",
                                 BM_Draw_GraphiteRecord);
    benchmark::RegisterBenchmark("BM_Draw_GraphiteSubmit",
                                 BM_Draw_GraphiteSubmit);
    benchmark::RegisterBenchmark("BM_Slot_UpdateTexture", BM_Slot_UpdateTexture)
        ->Arg(256)
        ->Arg(1024);
  }
#endif
