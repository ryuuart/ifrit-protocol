/** @file
 * WebImage's public surface: the slot painted through an SkCanvas over
 * its texture or bitmap, filled from raster pixels converted on the
 * caller's thread, blitted from a device texture, and invalidated so
 * the pages showing it repaint — every step on the web thread, the impl
 * travelling in the task.
 */
...
WebImage::~WebImage() {
  ...
  m_impl->engine->post([impl] {
    ...
    impl->bitmap = nullptr;
    if (impl->gpuTexture) {
      GpuDriver* driver = impl->engine->gpuDriver();
...
bool WebImage::paint(const std::function<void(SkCanvas&)>& painter) {
  ...
  m_impl->engine->postAndWait([impl, &painter, &ok] {
    if (impl->gpuTexture) {
      ...
      if (ok && impl->source) impl->source->Invalidate();
      return;
    }
    if (!impl->bitmap) return;
    if (void* pixels = impl->bitmap->LockPixels()) {
      SkImageInfo info =
          SkImageInfo::Make(impl->width, impl->height, kBGRA_8888_SkColorType,
...
bool WebImage::update(const SkPixmap& pixels) {
  ...
  SkImageInfo info =
      SkImageInfo::Make(m_impl->width, m_impl->height, kBGRA_8888_SkColorType,
                        kPremul_SkAlphaType, SkColorSpace::MakeSRGB());
  ...
  m_impl->engine->post([impl, converted] {
    ...
    if (void* dst = impl->bitmap->LockPixels()) {
      ...
      const size_t copyBytes = std::min(dstRowBytes, converted->rowBytes());
...
bool WebImage::update(const sk_sp<SkImage>& image) {
  ...
  SkPixmap pixmap;
  if (image->peekPixels(&pixmap)) return update(pixmap);
  ...
  SkImageInfo info =
      SkImageInfo::Make(image->width(), image->height(), kBGRA_8888_SkColorType,
                        kPremul_SkAlphaType, SkColorSpace::MakeSRGB());
  SkBitmap bitmap;
