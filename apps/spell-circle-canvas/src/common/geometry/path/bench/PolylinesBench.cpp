/** @file
 * The resampling currency under load: flattening an outline, resampling it
 * by count and by spacing, the smooth curve through a chain of controls,
 * the mark walk, the scanline lattice, the corner walk and the parallel
 * and displaced constructions, the noise a jitter starts from, and where
 * a pose along a contour comes from.
 */
