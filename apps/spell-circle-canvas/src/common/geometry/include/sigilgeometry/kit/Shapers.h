 ...
 * over a seam and does not grow it, and `Wave` answers both seams at
 * once so one value is both.
 *
 ...
 *  scalloped frame and ribbon edge is made of, and the BRAID primitive,
 *  since strands that oscillate trade sides and where they trade sides
 *  they cross. The third member is `phase`, not a zigzag flag: the
 *  cornered oscillation is `Zigzag` below.
 *  @trap As a PROFILE it is ZERO-MEAN, so it is a CENTRELINE and not a
 *  band width — a band asks for a width and this goes negative half the
 *  time, inverting the rails. `wavelength` is px, and the profile seam
 *  reads it as px-per-cycle on a nominal 1000 px contour. */
struct Wave {
