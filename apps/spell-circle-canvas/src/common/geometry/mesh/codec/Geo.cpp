/** @file
 * The Houdini .geo reader: the document parsed by simdjson into the
 * small tree this file addresses it through, then the file's point,
 * vertex, primitive and group classes decoded into one unwelded Part —
 * polygons fan-triangulated with each polygon vertex its own mesh
 * vertex, point attributes as lanes, primitive attributes as primitive
 * lanes, groups as 0/1 lanes named after the group, and a
 * primitive-less file as a point cloud. The attribute classes
 * themselves are decoded in GeoLanes.cpp.
 */
