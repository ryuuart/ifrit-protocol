/** @file
 * Benchmarks of the readers: OBJ, GLB and .geo decoded from bytes in
 * memory, per triangle or point.
 */
...
void countTriangles(benchmark::State& state, const codec::decode::Model& model,
                    size_t bytes) {
...
void BM_ImportObj(benchmark::State& state) {
  ...
  std::optional<codec::decode::Model> model;
  for ([[maybe_unused]] auto iteration : state) {
    model = codec::decode::model(obj.data(), obj.size(), "grid.obj");
...
void BM_ImportGlb(benchmark::State& state) {
  ...
  std::optional<codec::decode::Model> model;
  for ([[maybe_unused]] auto iteration : state) {
    model = codec::decode::model(glb.data(), glb.size(), "grid.glb");
    benchmark::DoNotOptimize(model);
  }
  if (!model) {
...
void BM_ImportGeo_Points(benchmark::State& state) {
  ...
  for ([[maybe_unused]] auto iteration : state)
    benchmark::DoNotOptimize(
        codec::decode::model(geo.data(), geo.size(), "p.geo"));
