/** @file
 * The shading terms this tier evaluates, each against the closed form a
 * device shader's own spelling of it is held to, and the environment they
 * are read out of: a constant sky, the backdrop the camera sees, the
 * ground projection that moves its horizon, and the panorama read that
 * cannot be keyed on an address.
 */
...
TEST(Shading, ThePolynomialInverseTrigonometryMeetsTheStandardLibrarys) {
...
TEST(Shading, AMetalReflectsItsOwnColourAndADielectricFourPerCent) {
  ...
  // Fresnel head on is the reflectance itself, and grazing is a mirror.
...
TEST(Shading, TheSplitSumIsExactForAMirrorAndTakesLessAsItRoughens) {
  using namespace sigil::geometry::mesh::render;
  // The split sum at its one exact point: a mirror head on returns the
  // radiance it was handed.
  EXPECT_NEAR(
      environmentSpecular(glm::vec3(1.0f), glm::vec3(1.0f), 0.0f, 1.0f).x, 1.0f,
      3e-3f);
  // A rough surface takes less of it than a smooth one.
  EXPECT_LT(
      environmentSpecular(glm::vec3(1.0f), glm::vec3(0.04f), 1.0f, 1.0f).x,
      environmentSpecular(glm::vec3(1.0f), glm::vec3(0.04f), 0.0f, 1.0f).x);
}

TEST(Shading, TransmissionAttenuatesOverDistanceAndStopsAtTotalReflection) {
  using namespace sigil::geometry::mesh::render;
