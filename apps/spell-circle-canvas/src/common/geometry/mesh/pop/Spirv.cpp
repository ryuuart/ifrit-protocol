/** @file
 * The kernel's SPIR-V, made to mean what its source says.
 *
 ...
 * twice — and the host build, whose contraction is pinned off, rounds
 * twice. One decoration per arithmetic result is what makes the two
 * agree, and without it every expression of the shape `a + b * c`
 * disagrees in its last place.
 *
 ...
 * second backend would have to remember to do this.
 */
...
std::vector<uint32_t> decorated(const uint32_t* words, size_t count) {
  std::vector<uint32_t> out(words, words + count);
  if (count < kHeaderWords || out[0] != 0x07230203u) return out;
  ...
  for (size_t at = kHeaderWords; at < count;) {
    ...
    if (length == 0 || at + length > count) return {words, words + count};
...
std::span<const uint32_t> spirv() {
  static const std::vector<uint32_t> module = decorated(
