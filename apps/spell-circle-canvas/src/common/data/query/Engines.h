...
struct Database::Impl {
  ...
  [[nodiscard]] virtual Access access() const = 0;
  [[nodiscard]] virtual std::optional<Table> query(std::string_view sql,
                                                   std::string* why) const = 0;
  [[nodiscard]] virtual std::optional<bool> writes(std::string_view sql,
                                                   std::string* why) const = 0;
...
/** An empty path opens a memory store, which is always read-write: a store
 *  nobody can write is a store with nothing in it. */
