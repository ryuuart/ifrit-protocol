 ...
 *  - geometry in is a sigil::geometry::mesh::Mesh, uploaded as it stands;
 ...
 *  - the camera is a sigil::geometry::mesh::camera::Camera, so a
 *    Skia-composited scene and a World render agree about where things
 *    sit.
 *
...
class World {
 public:
  ...
  uint32_t place(const geometry::mesh::Mesh& mesh, const glm::mat4& model,
  ...
  void setMesh(uint32_t id, const geometry::mesh::Mesh& mesh);
   ...
   *
   *  The kernel matches geometry::mesh::curve::Spline3's closed Catmull-Rom
   * parameter for parameter, so a CPU query of position(t) on the same loop
   * lands on the band. Width hangs world-vertical off the tangent.
   *
  ...
  uint32_t placeChain(const geometry::mesh::Mesh& stamp,
  ...
  uint32_t placeChainOn(uint32_t upstream, const geometry::mesh::Mesh& stamp,
                        const pop::Chain& chain, const Material& material);
  ...
  geometry::mesh::Cloud readChain(uint32_t id);
  ...
  uint32_t placeStamps(const geometry::mesh::Mesh& stamp,
   ...
   *  props. */
  void setStamps(uint32_t id, const geometry::mesh::Cloud& cloud,
  ...
  void setCamera(const geometry::mesh::camera::Camera& camera);
