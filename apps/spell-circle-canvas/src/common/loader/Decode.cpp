...
decodeWithOiio(const std::byte *bytes, size_t size,
  ...
  bitmap.setImmutable();
  return sigil::image::ImageAsset::wrap(bitmap.asImage());
}
...
decodeImage(const std::byte *bytes, size_t size,
  ...
  if (options.layer.empty()) {
    if (auto asset = sigil::image::ImageAsset::decode(
...
probeImage(const std::byte *bytes, size_t size,
           const std::filesystem::path &pathHint) {
  if (auto sniff = sigil::image::ImageAsset::probe(
          SkData::MakeWithoutCopy(bytes, size))) {
    ResourceInfo info;
    info.kind = ResourceInfo::Kind::Image;
    info.width = sniff->width;
    info.height = sniff->height;
    info.channels = 4; // the N32 the decode path lands on
    info.frames = sniff->frames;
    info.format = std::move(sniff->format);
    return info;
  }
#ifdef SIGILLOADER_HAS_OIIO
