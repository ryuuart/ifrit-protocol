/** @file
 * Formation and painting cost for one representative procedural brush mark.
 */
...
void BrushStroke(benchmark::State& state) {
  ...
  frame.width = 640;
  frame.height = 480;
  const brush::Brush tool =
      brush::watercolor({0.12f, 0.36f, 0.72f, 1.0f}, 22.0f);
  const brush::fields::Curl field(91);
...
void BrushHatch(benchmark::State& state) {
  ...
  brush::Brush tool = brush::pencil({0.14f, 0.21f, 0.31f, 1.0f}, 2.0f);
...
void BrushMass(benchmark::State& state) {
  ...
  box.scale(3.5f);
  brush::Brush tool = *box.find("crayon");
