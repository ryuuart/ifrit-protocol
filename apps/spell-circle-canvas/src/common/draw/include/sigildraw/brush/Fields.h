 ...
 *  smooth noise field. Scale sets the size of its eddies and drift moves
 *  the field through its third dimension over time. The field is the
 *  value's own, seeded here: a pen's `noiseSeed` does not reach it, so two
 *  curls with one seed agree everywhere whichever pen paints them. */
class Curl {
 public:
 ...
 *  positive and outward when negative; direction is one for clockwise on
 *  the canvas and minus one for anticlockwise. */
struct Vortex {
  SkPoint center{0, 0};
...
/** Parallel flow whose direction oscillates across the canvas and in
 *  time. */
struct Wave {
...
/** The seven named fields every engine starts with: `hand`, `curved`,
 *  `zigzag`, `waves`, `seabed`, `spiral` and `columns`. */
[[nodiscard]] std::vector<std::pair<std::string, Direction>> stockFields();
