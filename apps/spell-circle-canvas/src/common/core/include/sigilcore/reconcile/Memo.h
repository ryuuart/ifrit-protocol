...
/** A deferred describe and its key. The reconciler compares `environment` first
 *  and `properties` second against the memo the retained node was last
 *  described from; on a hit the retained payload stands and `invoke` is
 *  never called, on a miss `invoke` runs under `environment` re-established and
...
struct Memo {
  ...
  /** The `environment::` bindings in scope where this memo was WRITTEN. The
   * memo is the one deferred describe, so it is also the one place an inherited
   * value could go stale: the snapshot rides in the memo's key and is
   * re-established around the invoke. Empty — hence free — when nothing is
   * bound. */
