...
/** @file
 * Comparable type erasure — Erased, the one shape a seam value on a
 * description takes: a set of operations behind an abstract interface,
 * held with the value that implements them so two descriptions can ask
 * whether they carry the same one.
 */
