 ...
 *
 * `Noise.h` answers a number for an INDEX, where neighbouring indices
 * are unrelated. A field answers a number for a POSITION, and points
 * near each other read near values, which is what a displacement, a
 * drift, a grain and a flow want. It is built on the same
 * `noise::lattice` mixer, so the two agree about what a seed means.
 *
 * It is ONE VALUE WITH PROPS, not a header per kind: the kind, the
 * octaves, the fold, the period and the warp are all members, so the
 * value compares exactly and a look chosen once is carried as a token.
 */
...
/** PERLIN GRADIENT NOISE in two dimensions, in [-1, 1]: zero at every
 *  lattice point and shaped by the direction each corner pulls, which
 *  is what removes the blobbiness of value noise. The interpolated dot
 *  product of unit gradients is bounded by half the square root of two,
 *  so the result is scaled by its reciprocal to reach the ends. */
...
/** SIMPLEX NOISE in two dimensions, in about [-1, 1]: the same
 *  gradients read on a triangular lattice, where three corners
 *  contribute instead of four and no direction lines the cell
 *  boundaries up — the axis-aligned ridging a square lattice leaves.
 *  @silent a `period` is stated: the skew that turns squares into
 *  triangles carries no Cartesian period, so it is ignored rather than
 *  producing a seam. */
...
/** A NOISE LOOK AS ONE VALUE, read at a point: the ten numbers a grain,
 *  a drift, a flow or an erosion is set by, chosen once for a drawing
 *  far more often than per call. It compares exactly, member for
 *  member, so a memo keyed on one may be skipped and a theme may bind
 *  one. The range is [-1, 1] at `Fold::None` and `Fold::Ridged` and
 *  [0, 1] at `Fold::Turbulence`, whatever the octave count.
 *  @trap `Gradient` and `Simplex` approach the ends of the range
 *  without a hard bound, and neither is clamped. */
struct Field {
