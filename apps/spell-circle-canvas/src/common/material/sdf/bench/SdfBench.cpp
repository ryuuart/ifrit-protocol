...
Material styled(sdf::SdfShape kind) {
  sdf::Style style;
  style.fill = {0.2f, 0.4f, 0.9f, 1};
  style.borderWidth = 2;
  style.glowRadius = 6;
  const sdf::Shape shape = kind == sdf::SdfShape::Circle     ? sdf::circle()
                           : kind == sdf::SdfShape::RoundBox ? sdf::roundBox(8)
                                                             : sdf::star(5, 3);
  return sdf::material(shape, style);
}

void SdfShader(benchmark::State& state) {
  const Material m = styled((sdf::SdfShape)state.range(0));
...
void SdfPaint(benchmark::State& state) {
  ...
  const Material m = styled(sdf::SdfShape::Star);
