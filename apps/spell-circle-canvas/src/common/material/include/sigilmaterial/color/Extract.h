 ...
 * the colours that stand for them out. Pixels rather than an image,
 * because the algorithm has nothing to do with how a picture was decoded
 * and this leaf names no renderer; the crossing from a decoded image
 * lives with the renderer that owns it.
 */
...
/** HOW THE COLOURS ARE CHOSEN. Both answer a table of the requested
 *  size and neither is random. `KMeans` moves the entries until each is
 *  the average of the pixels closest to it, so its colours are the ones
 *  a picture actually holds; `MedianCut` divides the colours into
 *  equal-population boxes, so its entries are spread evenly over the
 *  range the picture covers. Reach for the first when the table has to
 *  look like the picture and the second when it has to cover it. */
...
/** THE TABLE @p pixels IS MADE OF. The work happens in OKLab, so
 *  "closest" means what an eye reports rather than what the sRGB numbers
 *  do. DETERMINISTIC: the same pixels answer the same table, every run
 *  and every platform, because the first entries are chosen by taking
 *  the pixel farthest from every entry so far rather than at random. */
