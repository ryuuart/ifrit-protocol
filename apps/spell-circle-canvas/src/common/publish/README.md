...
library, `SigilPublish`, owns the platform protocol implementations. Its C++
headers use opaque graphics handles and depend on no window toolkit, drawing
library, sketch runtime or resource hub. Namespace `sigil::publish`.
...
auto publisher = sigil::publish::createPublisher(
    "Live Canvas", sigil::publish::Backend::Metal, nativeDevice);
...
`sigil::publish::createPublisher` returns null for an empty name, a missing
...
`sigil::publish::Backend::Metal` selects Syphon on macOS. The device is an
...
`sigil::publish::Backend::Direct3D11` selects Spout on Windows when its SDK is
...
`sigil::publish::Publisher::name` reads back the publication's name.
...
`sigil::publish::Publisher::publishFrame` borrows the input texture for the
...
* `Subscription.h` — `Subscription`, `subscribe`,
  `defaultMetalDevice`

```cpp
#include <sigilpublish/Subscription.h>

auto incoming = sigil::publish::subscribe(
    "Live Canvas", "", sigil::publish::defaultMetalDevice());
if (incoming)
...
`sigil::publish::subscribe` currently receives Syphon on Metal. It returns
...
and `sigil::publish::Subscription::newestFrame` reconnects when a publisher
...
`sigil::publish::Subscription::standing` checks both client validity and the
publication directory. `sigil::publish::Subscription::generation` counts
arrivals across reconnections. `sigil::publish::Subscription::name` is the
requested name; `sigil::publish::Subscription::publishingApplication` is the
application the current publication names, empty until connected.

`sigil::publish::defaultMetalDevice` returns the process's shared system Metal
...
`tools/receiver/` contains the macOS AppKit/Metal viewer and capture tool,
built as `build/bin/<config>/Receiver.app`. It depends on this library, the
image encoder and byte sink; it links no Qt or sketch runtime.

```sh
Receiver --list
Receiver "Live Canvas" [--app Sketchbook]
Receiver "Live Canvas" --grab frame.png [--frames 3] [--timeout 10]
```

Listing lets the run loop hear existing publications. The window waits for
the requested publication and follows it across restarts. `--grab` waits for
the requested number of frame notifications, reads the newest texture back
and writes a PNG. A retained static publication can satisfy a one-frame
grab; multiple frames require subsequent publications. Exit status 2 means
no matching publication, 3 means the frame wait expired, and 4 means writing
failed. The executable preserves this diagnostic interface independently of
any application's preview UI.

## Build and tests

```sh
cmake --build build --config Release --target SigilPublish publish_test Receiver
ctest --test-dir build -C Release -R '^(Publish|Receiver)' --output-on-failure
```

`publish_test` covers factory refusals, receiver arguments, texture-to-file
row and channel order, and a static publication received by a later client.
GPU cases are labelled `gpu` and require Metal. Cross-process receiver grabs
validate application publication, including resize and late subscription.
