...
std::vector<Publication> syphonPublications() {
  ...
  @autoreleasepool {
    for (NSDictionary<NSString *, id> *server in
         [[SyphonServerDirectory sharedDirectory] servers]) {
      NSString *name = server[SyphonServerDescriptionNameKey];
      NSString *application = server[SyphonServerDescriptionAppNameKey];
      result.push_back({name ? name.UTF8String : "", application ? application.UTF8String : ""});
    }
  }
  return result;
}
