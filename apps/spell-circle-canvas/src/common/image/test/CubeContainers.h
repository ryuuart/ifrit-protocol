...
inline std::vector<std::byte> cubeDds(const CubeFaces& faces, int edge) {
  ...
  put32(out, 0x1 | 0x2 | 0x4 | 0x8 |
                 0x1000);          // caps, height, width, pitch, pixelformat
  ...
  put32(out, 0x200 | 0x400 | 0x800 | 0x1000 | 0x2000 | 0x4000 |
                 0x8000);  // cube, all six
  put32(out, 0);           // dwCaps3
  put32(out, 0);           // dwCaps4
  ...
  for (SkColor c : faces)
    for (int i = 0; i < edge * edge; ++i)
      put32(out, ((uint32_t)SkColorGetA(c) << 24) |
                     ((uint32_t)SkColorGetR(c) << 16) |
                     ((uint32_t)SkColorGetG(c) << 8) |
                     (uint32_t)SkColorGetB(c));
  return out;
}
