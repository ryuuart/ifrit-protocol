...
constexpr SkColor4f kPaper = hex(0xefe1de);   // aged ivory, pink undertone
...
struct NightingaleCoxcomb : sigil::compose::sketch::Sketch {
  ...
  Element wheel(sketch::SketchContext &ctx, const std::array<Month, 12> &data,
    ...
    // The 12 hairline spokes: only the hub carries them on the plate —
    // outside it the wedge outlines already draw every radial edge.
    const float hubFrac = 0.30f;
    for (int i = 0; i < 12; ++i) {
      wheelBox.child(
          box()
              .absolute()
              ...
              .outline(spoke(hubFrac, (float)i * 30.0f))
  ...
  Element describe(sketch::SketchContext &ctx) {
    ...
    root.child(box().absolute().left(938).top(0).width(24).height(kH).fill(
        linearGradient({0, 0}, {24, 0},
                       {hex(0x3a2a20, 0.0f), hex(0x3a2a20, 0.10f),
    ...
    auto ringLabels = [&](const std::array<Month, 12> &data, SkPoint centre,
                          float rMax, float startSec, const char *tag) {
      const float floorR = rMax * 0.30f + 26.0f;
      const float gap = rMax > 400 ? 26.0f : 20.0f;
      const float step = rMax > 400 ? 24.0f : 20.0f;
      for (int m = 0; m < 12; ++m) {
        const Month &mo = data[m];
        const float rim =
        ...
        arcRun(labels, ctx, labelStyle, centre, mo.label, bearing,
               base + (twoLines ? step : 0.0f), false, 0.6f, delay,
               std::string(tag) + "L" + std::to_string(m) + "a");
        if (twoLines)
          arcRun(labels, ctx, labelStyle, centre, mo.line2, bearing, base,
                 false, 0.6f, delay + 60.0f,
                 std::string(tag) + "L" + std::to_string(m) + "b");
      }
    };
    ringLabels(kD1, kC1, kR1, tLabel1, "a");
    ringLabels(kD2, kC2, kR2, tLabel2, "b");
