/** @file
 * The volatility walk: what may cache and at which tier — the verdict one
 * node hands its subtree, and the tiers a memo can hold across.
 */
...
core::SubtreeVerdict Composer::Impl::computeVolatile(Instance& inst,
  ...
  const bool typeBlends = [&] {
    ...
    for (const SpanRestyle& span : text.spanRestyles) {
      // A span's passes blend or not on their own; what it is laid over is
      // the leaf's, already judged above.
      const sigil::weave::PaintStyle paint =
          sigil::weave::overlay(sigil::weave::TextStyle{}, span.partial).paint;
