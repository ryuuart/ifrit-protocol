...
TEST(ComposePlacers, RepeaterLawExponentialScaleLinearEverythingElse) {
  ...
  // and tints[].a stays exactly what the author put there. Folding one into
  ...
  EXPECT_FLOAT_EQ(pool.tints()[3].a, 1.0f);  // untouched
}
...
TEST(ComposePlacers, TheAlphaLaneFadesWithoutTouchingTheTint) {
  ...
  // place::repeat writes IT rather than tints[].a. Sharing one lane would
