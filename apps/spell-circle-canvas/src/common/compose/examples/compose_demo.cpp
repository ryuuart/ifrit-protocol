...
Element poster(SkSize size) {
  return box()
      ...
      .child(box()
                 ...
                 .child(text(u8"summer program", styleAt(26, 0xffc9d1e8))))
...
Element blendPanel(SkSize size) {
  std::vector<Row> a = {
      {"alpha", 42, true}, {"beta", 17, false}, {"gamma", 8, false}};
  std::vector<Row> b = {
      {"delta", 99, false}, {"epsilon", 64, true}, {"zeta", 31, false}};
  auto tableA = box().column().gap(6).padding(20).width(360).children(
      std::vector<Element>{});  // filled below
  for (const Row& r : a) tableA.child(scoreRow(r));
  auto tableB = box().column().gap(6).padding(20).width(360);
  for (const Row& r : b) tableB.child(scoreRow(r));
      ...
      .child(
          box()
              ...
              .child(box()
                         ...
                         .child(text(
                             u8"PathFormat stamps a leaf around the frame,",
                             styleAt(17, 0xff9aa4bb)))
                         .child(text(
                             u8"a ContourWalk orbits dots along the outline,",
                             styleAt(17, 0xff9aa4bb)))
                         .child(text(u8"and the divider below is two numbers "
                                     u8"of dash data.",
                                     styleAt(17, 0xff9aa4bb)))
                         .child(box().height(30).foreground(dashedBottom))));
}
  ...
  return stack()
      ...
      .child(
          box()
              ...
              .child(text(
                  u8"plus-blended scanline layers brighten where they stack;",
                  styleAt(17, 0xff9aa4bb)))
              .child(text(
                  u8"the halo is the same headline, blurred and plus-blended.",
                  styleAt(17, 0xff9aa4bb))))
      .child(box()
                 .column()
                 .padding(70)
...
Element organicPanel(SkSize size) {
  ...
  for (int i = 0; i < 12; ++i)
    chaos.child(box()
                    .width(80 + (float)(i % 5) * 24)
                    .height(64 + (float)(i % 4) * 22)
                    .shape(shapes::blob((uint32_t)(300 + i), 0.34f, 5 + i % 6))
  ...
  auto sigilStar =
      box()
          ...
          .child(layout(layouts::Radial{.radiusFraction = 0.5f})
                     .inset(0)
                     .children([&] {
                       ...
                       for (int i = 0; i < 10; ++i)
                         beads.push_back(
                             box()
                                 ...
                                 .fill(Fill::color({0.2f, 0.1f, 0.14f, 0.9f})));
                       return beads;
                     }()));
  ...
  auto plaque =
      box()
          ...
          .child(text(u8"stamped-star border above", styleAt(16, 0xffe8d9c2)));
...
int main(int argc, char** argv) {
...

  {
    ...
    if (!writePanel(composer, size, outDir / "poster.png")) return 1;
  ...
  }
  {
    ...
    if (!writePanel(composer, size, outDir / "scoreboard.png")) return 1;
  }
  {
    Composer composer(ticker, fonts());
    SkSize size = {640, 480};
    ...
    if (!writePanel(composer, size, outDir / "stack_blend.png")) return 1;
  }

  {
    ...
    if (!writePanel(composer, size, outDir / "chrome.png")) return 1;
  }

  {
    ...
    if (!writePanel(composer, size, outDir / "crt_bloom.png")) return 1;
  }

  {
    ...
    if (!writePanel(composer, size, outDir / "organic.png")) return 1;
...

  {
    ...
    if (!writePanel(composer, size, outDir / "tile_maze.png")) return 1;
  }
