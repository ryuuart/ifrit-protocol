// The instances tier's tests: the pool's lanes, the atlas bake and its
// variants, the stamp in both modes, the per-sprite blend, the pick, and the
// placers. Links SigilComposeInstances alone over the kernel, so a test
// reaching for a catalog fails to link rather than passing on a dependency
// the tier does not have.

#include "support/InstancesTestSupport.h"
