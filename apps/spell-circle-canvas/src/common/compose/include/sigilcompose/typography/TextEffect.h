...
/** @file
 * SigilCompose typography — the EFFECT: what a glyph sees (`GlyphInfo`),
 * one glyph's deviation from rest (`GlyphMod`), the comparable value that
 * produces it (`TextEffect`, with `Phase` for a sequence), and the
 * effects the runtime evaluates by structure rather than by calling a
 * preset's body — the substitution, the shader pass, the keyframe table,
 * the hold and the two combinators, under `fx::`. The stock presets that
 * are plain values over this seam are the kit's, in
