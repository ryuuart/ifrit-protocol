 ...
 *
 *  Each child says which cells it takes with `Element::gridCells` and
 *  where it sits in them with `Element::gridCellAlign`. A child that says
 *  nothing flows into the next free cell, left to right and then down.
 *
 *      layout(layouts::Table{.width = 500, .spacing = 2, .padding = 1})
 *          .children({masthead()
 *                         .gridCells(0, 0, 5, 1)
 *                         .gridCellAlign(Align::End, Align::Start)})
 *          .children({panel().gridCells(1, 0, 1, 2)})
 *
