 ...
 * rhythm), `Scatter` (seeded jitter) and `Table` (the HTML auto table:
 * unequal columns sized by their content, spans, and a surplus shared in
 * proportion).
 *
 ...
 * their cell span, and `Table`, which sizes a child to its cell only where
 * the child asked to be stretched to it. Placement is pure arithmetic over `LayoutInput`, so the
