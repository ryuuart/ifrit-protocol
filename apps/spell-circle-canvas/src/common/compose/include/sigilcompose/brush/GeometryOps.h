 ...
 * outline: the GeometryOp that a comparable shaper and a raw path
 ...
 *  `geometry::path::ops::PathOp` — the recipe step every distort in that
 *  catalogue converts to, chained with `geometry::path::ops::chain`. It
 *  can do anything, and it can never prune: an incomparable callable
 *  compares conservatively unequal, so a node wearing one re-records
 *  every render (memo the host, or keep it pointer-stable).
 *
 ...
 *  assigned to a `geometry::path::ops::PathOp` first, because two
...
class GeometryOp {
 public:
  GeometryOp(geometry::path::ops::PathOp fn)  // NOLINT: escape hatch
      : m_apply(std::move(fn)) {}
