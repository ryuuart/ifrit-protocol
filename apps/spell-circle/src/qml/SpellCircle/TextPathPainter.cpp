...
static QPainterPath buildOffsetPath(const QPainterPath &path, float perpOffset) {
    ...
    for (int i = 0; i <= n; ++i) {
        ...
        const float ox =
            static_cast<float>(pos.x()) + std::sin(angle) * perpOffset;
        const float oy =
            static_cast<float>(pos.y()) - std::cos(angle) * perpOffset;
        if (i == 0)
            result.moveTo(ox, oy);
        else
            result.lineTo(ox, oy);
...
void TextPathPainter::paint(QCanvasPainter *p, const QPainterPath &path,
    ...
    case PathAlignment::Left:   arcPos = anchor; break;
    case PathAlignment::Right:  arcPos = anchor - totalWidth; break;
    default:                    arcPos = anchor - totalWidth * 0.5f; break;
    }
